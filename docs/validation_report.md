# ShadeSense AI Validation Report

Use this template for manual validation. Do not fill results unless the case was
actually tested locally with an authorized image.

## Manual Test Matrix

| Case ID | Image Filename | Skin-Depth Group | Lighting Condition | Face Detected | Regions Used | Regions Excluded/Down-Weighted | Extraction Quality | Match Confidence | Top 3 Shades | Visual Mask Quality | Result Looks Reasonable? Yes/No | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| V-001 |  | Fair | Daylight |  |  |  |  |  |  |  |  |  |
| V-002 |  | Light | Daylight |  |  |  |  |  |  |  |  |  |
| V-003 |  | Medium | Daylight |  |  |  |  |  |  |  |  |  |
| V-004 |  | Tan | Daylight |  |  |  |  |  |  |  |  |  |
| V-005 |  | Deep | Daylight |  |  |  |  |  |  |  |  |  |
| V-006 |  | Rich-deep | Studio/highlight |  |  |  |  |  |  |  |  |  |
| V-007 |  | Any | Yellow indoor lighting |  |  |  |  |  |  |  |  |  |
| V-008 |  | Any | Strong shadow |  |  |  |  |  |  |  |  |  |
| V-009 |  | Any | Daylight |  |  |  |  |  |  |  |  | Side-angle / side pose |
| V-010 |  | Any | Daylight |  |  |  |  |  |  |  |  | Sunglasses or occlusion |
| V-011 |  | Any | Any |  |  |  |  |  |  |  |  | No-face image |
| V-012 |  | Any | Any |  |  |  |  |  |  |  |  | Multi-face image |

## Items To Record

- Selected catalog:
- Catalog shade count:
- Extracted RGB/Lab:
- Estimated ITA and skin-depth category:
- Lighting quality score and warnings:
- Included regions:
- Excluded regions:
- Reduced-weight regions:
- Stable patches per region:
- Highlight/shadow patches rejected:
- Extraction source:
- Top 3 shades and Delta E:
- Depth sanity notes:
- Confidence breakdown per shade:
- Close-match tie shown?:
- User-observed plausibility:
